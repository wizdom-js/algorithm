    ...
    while queue:
        ...
        for i in range(4):
            ...
            if 0 <= ny < h and 0 <= nx < w: # 움직일 곳이 범위 내라면
                if cheese[ny][nx] == 0 and not visited[ny][nx]: # 치즈가 없는 칸(0)이고, 방문하지 않은 곳이라면
                    visited[ny][nx] = True  # 방문처리
                    queue.append((ny, nx))  # 해당 위치에서 검토해보기 위해 queue에 넣어줌 (0에서 만난 치즈는 다 가장자리 치즈이므로)
